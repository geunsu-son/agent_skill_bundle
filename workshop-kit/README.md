...
1. **Bundle Catalog gate**만 먼저 `.cursor/`에 설치
2. gate가 [`catalog.md`](catalog.md)의 후보를 보여 줌
3. 사용자가 고른 번들만 번들 소스에서 가져와 설치
...
```text
workshop-kit/
├── catalog.md                              등록된 에이전트 번들 목록 (소스 catalog)
├── rules/
│   ├── agent-skill-workshop.mdc            공방 운영 원칙
│   └── bundle-catalog.mdc                  gate — 번들 선택·관리 원칙
└── skills/
    ├── idea-to-agent-artifact/
    │   └── SKILL.md                        아이디어 → 최소 번들 구현
    └── manage-agent-bundles/
        └── SKILL.md                        gate 연결·번들 설치 절차
```

| 번들 | Rule | Skill | 역할 |
|---|---|---|---|
| Agent Skill Workshop | `agent-skill-workshop.mdc` | `idea-to-agent-artifact` | 아이디어를 번들로 구현 |
| Bundle Catalog | `bundle-catalog.mdc` | `manage-agent-bundles` | gate — 번들 선택·관리 |

등록 상태와 gate 순서는 [`catalog.md`](catalog.md)를 기준으로 합니다.
...
작업 중인 다른 저장소에 처음 연결할 때는 루트 [README의 시작하기](../README.md#시작하기) 프롬프트를 사용합니다.
