...
완성된 MCP 프레임워크가 아닙니다. 사내 `purple-context-mcp` README를 참고 경로로 두고, 다음 서버를 같은 기준으로 안내하기 위한 `testing` 예시입니다.
...
`testing` — `purple-context-mcp` README로 참고 경로를 고쳤습니다. 이 저장소 구현은 여기서 마감합니다. 실제 MCP 제작으로 절차를 검증할 예정입니다.

전체 소스·차트·서버 구현은 이 저장소에 두지 않습니다. `.cursor/` 승격은 하지 않습니다.
