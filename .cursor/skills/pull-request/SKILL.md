...
`core/user-rule/user_rule.md` §9의 선언적 원칙을 지키고, 이 Skill은 실행 순서·형식·검증을 담당한다.
...
- base는 보통 `main`이다.
- 이 저장소의 `core/user-rule/README.md`는 rule 변경 PR에 한해 `Why` / `Changes` / `Expected behavior` 형식을 권장한다. 일반 코드 PR은 위 기본 Template을 따른다.
